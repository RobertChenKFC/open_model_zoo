...
class AlphaIoU(BaseBackgroundMattingMetrics):
    __provider__ = 'alpha_iou'

    def update(self, annotation, prediction):
        pred = self.get_prediction(prediction) >= 128
        gt = self.get_annotation(annotation) > 0
        value = (
            np.float32(np.sum(np.logical_and(pred, gt))) /
            np.float32(np.sum(np.logical_or(pred, gt)))
        )
...
class MeanOfAbsoluteDifference(BaseBackgroundMattingMetrics):
    ...
    def update(self, annotation, prediction):
        pred = self.get_prediction(prediction) / 255.0
        gt = self.get_annotation(annotation)
        value = np.mean(abs(pred - gt))
        self.results.append(value)
        return value
